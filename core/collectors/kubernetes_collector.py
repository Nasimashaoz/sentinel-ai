...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        ...
        if hasattr(event, "metadata") and event.metadata:
            namespace = getattr(event.metadata, "namespace", namespace)

        if hasattr(event, "involved_object") and event.involved_object:
            name = getattr(event.involved_object, "name", name)
            kind = getattr(event.involved_object, "kind", kind)
            # Try to get namespace from involved_object if not found in metadata
            if namespace == "unknown":
                namespace = getattr(event.involved_object, "namespace", namespace)
