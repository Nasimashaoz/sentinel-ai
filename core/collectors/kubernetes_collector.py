...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        ...
        namespace = getattr(event.metadata, 'namespace', None) or getattr(event.involved_object, 'namespace', "")
        ...
        if reason in ("ErrImagePull", "ImagePullBackOff", "BackOff") and "image" in msg.lower():
            return {
        ...
        # Pod crash / OOMKilled
        elif reason in ("OOMKilling", "Killed", "Failed") or (reason == "BackOff" and "image" not in msg.lower()):
            return {
                "type": "K8S_POD_CRASH",
                "source_ip": "kubernetes",
                "service": f"k8s:{namespace}/{name}",
