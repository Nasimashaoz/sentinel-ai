...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        ...
        # Pod crash / OOMKilled
        if reason in ("BackOff", "OOMKilling", "Killed", "Failed") and "pulling image" not in msg.lower():
            return {
                "type": "K8S_POD_CRASH",
                ...
                "reason": reason,
                "raw": f"{reason}: {msg} for {name}",
                "timestamp": event_time.isoformat(),
            }

        # Image pull failure — possible supply chain attack
        if reason in ("ErrImagePull", "ImagePullBackOff", "BackOff") and "pulling image" in msg.lower():
            return {
