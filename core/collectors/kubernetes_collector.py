...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        ...
        # Test mocks may not have a fully formed metadata object
        namespace = "default"
        if hasattr(event, "metadata") and hasattr(event.metadata, "namespace"):
            namespace = event.metadata.namespace
        elif hasattr(event, "involved_object") and hasattr(event.involved_object, "namespace"):
        ...
        if reason in ("BackOff", "OOMKilling", "Killed", "Failed"):
            # If reason is BackOff but message mentions image pulling, it's K8S_IMAGE_PULL_BACKOFF
