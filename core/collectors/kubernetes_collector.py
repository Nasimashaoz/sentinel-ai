...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        event_time = event.last_timestamp or event.event_time
        if not event_time:
            # Tests might use mock events with None timestamp
            event_time = datetime.now(timezone.utc)

        reason = event.reason or ""
        msg = event.message or ""
        namespace = getattr(event.involved_object, "namespace", "")
        name = getattr(event.involved_object, "name", "")
        kind = getattr(event.involved_object, "kind", "")

        # Pod crash / OOMKilled
        if reason in ("BackOff", "OOMKilling", "Killed", "Failed"):
            # If reason is BackOff, check message to distinguish between Pod BackOff and ImagePullBackOff
            if reason == "BackOff" and "pulling image" in msg.lower():
                return {
            ...
            return {
                "type": "K8S_POD_CRASH" if reason != "OOMKilling" else "K8S_OOM_KILLED",
                "source_ip": "kubernetes",
                "service": f"k8s:{namespace}/{name}",
                "namespace": namespace,
                "resource": f"{kind}/{name}",
        ...
        if reason in ("ErrImagePull", "ImagePullBackOff"):
            return {
                "type": "K8S_IMAGE_PULL_FAILURE" if reason != "ImagePullBackOff" else "K8S_IMAGE_PULL_BACKOFF",
                "source_ip": "kubernetes",
                "service": f"k8s:{namespace}/{name}",
                "namespace": namespace,
                "resource": f"{kind}/{name}",
