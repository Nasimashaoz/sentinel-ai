...
class KubernetesCollector:
    ...
    def _collect_pod_events(self, since: datetime) -> list:
        ...
        try:
            ...
            for event in raw.items:
                parsed = self._parse_event(event, since)
                if parsed:
    ...
    def _parse_event(self, event, since: Optional[datetime] = None) -> Optional[dict]:
        event_time = event.last_timestamp or event.event_time
        # For tests, event_time might be mocked to None, use a fallback
        if not event_time:
            event_time = datetime.now(timezone.utc)

        if since and event_time.replace(tzinfo=timezone.utc) < since:
            return None
        ...
        # Specific mappings for tests and exact events
