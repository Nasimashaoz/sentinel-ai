...
class KubernetesCollector:
    ...
    def _parse_event(self, event) -> Optional[dict]:
        ...
        namespace = event.involved_object.namespace or (event.metadata.namespace if hasattr(event, "metadata") and event.metadata else "")
        name = event.involved_object.name or ""
        kind = event.involved_object.kind or ""

        # Pod crash / OOMKilled
        if reason in ("OOMKilling", "Killed", "Failed", "BackOff") and "Back-off pulling image" not in msg and "ErrImagePull" not in reason and reason != "ImagePullBackOff":
            return {
                "type": "K8S_POD_CRASH",
        ...
        if reason in ("ErrImagePull", "ImagePullBackOff") or "Back-off pulling image" in msg:
            return {
